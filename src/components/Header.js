...
const Header = () => {
  ...
  return (
    <header className={`fixed w-full z-10 transition-all ${isActive ? 'bg-white' : 'bg-white shadow-md'}`}>
      <div className='container mx-auto py-8 px-8 flex items-center justify-between' style={{ padding: '2em' }}>
        {/* Logo */}
        <Link to={'/'}>
          <img className='w-16 h-auto' src={Logo} alt="Logo" />
        </Link>
        {/* Search bar */}
        <form onSubmit={handleSearch} className="ml-8 md:ml-0 flex">
        <input 
          type="text" 
          placeholder="Tìm kiếm..." 
          className="py-2 rounded-lg border-gray-300 border focus:outline-none focus:border-gray-400 mr-2"
          value={searchTerm}
