            ...
            <div className='w-full min-h-[150px] flex items-center gap-x-4'>
              ...
              <div className='w-full flex flex-col'>
                ...
                <div className='flex justify-between mb-2'>
                  {/* title */}
                  <Link to={`/product/${item.product_id}`} className='text-sm uppercase font-medium max-w-[240px] text-primary hover:underline'>{item.title}</Link>
                  {/* remove icon */}
                  <div onClick={() => removeFromCart(item.product_id)} className='text-xl cursor-pointer'>
                ...
                <div className='flex gap-x-2 h-[36px] text-sm'>
                  ...
                  <div className='flex flex-1 max-w-[100px] items-center h-full border text-primary font-medium'>
                    {/* minus icon */}
                    <div onClick={() => decreaseAmount(item.product_id)} className='flex-1 flex justify-center items-center cursor-pointer h-full '>
                      <IoMdRemove />
                    </div>
                    {/* amount */}
                    <div className='h-full flex justify-center items-center px-2'>{item.amount}</div>
                    {/* plus icon */}
                    <div onClick={() => increaseAmount(item.product_id)} className='flex-1 h-full flex justify-center items-center cursor-pointer'>
                      <IoMdAdd />
                    </div>
                  </div>
                  {/* item price */}
